...
use {
    crate::{
        state::{LockType, OpenPositionBumps, OpenPositionWithMetadataBumps, WhirlpoolBumps},
        util::RemainingAccountsInfo,
    },
    instructions::*,
};
...
pub mod whirlpool {
    ...
    ///   protocol fees.
    /// - `reward_emissions_super_authority` - Authority authorized to set
    ///   reward authorities in pools.
    ...
    /// Fee rate is set to the default values on the config and supplied
    /// fee_tier.
    ///
    /// ### Parameters
    /// - `bumps` - The bump value when deriving the PDA of the Whirlpool
    ///   address.
    ...
    /// `SqrtPriceOutOfBounds` - provided initial_sqrt_price is not between
    /// 2^-64 to 2^64
    ...
    /// Initializes a tick_array account to represent a tick-range in a
    /// Whirlpool.
    ///
    /// ### Parameters
    /// - `start_tick_index` - The starting tick index for this tick-array. Has
    ///   to be a multiple of TickArray size & the tick spacing of this pool.
    ///
    /// #### Special Errors
    /// - `InvalidStartTick` - if the provided start tick is out of bounds or is
    ///   not a multiple of TICK_ARRAY_SIZE * tick spacing.
    pub fn initialize_tick_array(ctx: Context<InitializeTickArray>, start_tick_index: i32) -> Result<()> {
    ...
    /// - `default_fee_rate` - The default fee rate that a pool will use if the
    ///   pool uses this fee tier during initialization.
    ///
    /// #### Special Errors
    ...
    ///
    /// ### Authority
    /// - "reward_authority" - assigned authority by the reward_super_authority
    ///   for the specified reward-index in this Whirlpool
    ///
    /// ### Parameters
    ...
    ///   lowest uninitialized index in this pool, or exceeds NUM_REWARDS, or
    ...
    ///
    /// ### Parameters
    /// - `reward_index` - The reward index (0 <= index <= NUM_REWARDS) that
    ///   we'd like to modify.
    /// - `emissions_per_second_x64` - The amount of rewards emitted in this
    ///   pool.
    ///
    /// #### Special Errors
    /// - `RewardVaultAmountInsufficient` - The amount of rewards in the reward
    ///   vault cannot emit more than a day of desired emissions.
    /// - `InvalidTimestamp` - Provided timestamp is not in order with the
    ///   previous timestamp.
    /// - `InvalidRewardIndex` - If the provided reward index doesn't match the
    ///   lowest uninitialized index in this pool, or exceeds NUM_REWARDS, or
    ///   all reward slots for this pool has been initialized.
    ...
    /// Open a position in a Whirlpool. A unique token will be minted to
    /// represent the position in the users wallet. The position will start
    /// off with 0 liquidity.
    ///
    /// ### Parameters
    ...
    ///
    /// #### Special Errors
    /// - `InvalidTickIndex` - If a provided tick is out of bounds, out of order
    ///   or not a multiple of the tick-spacing in this pool.
    ...
    /// Open a position in a Whirlpool. A unique token will be minted to
    /// represent the position in the users wallet. Additional Metaplex
    /// metadata is appended to identify the token. The position will start
    /// off with 0 liquidity.
    ///
    /// ### Parameters
    /// - `tick_lower_index` - The tick specifying the lower end of the position
    ...
    pub fn open_position_with_metadata(
        ...
        instructions::open_position_with_metadata::handler(ctx, bumps, tick_lower_index, tick_upper_index)
    }

    /// Add liquidity to a position in the Whirlpool. This call also updates the
    /// position's accrued fees and rewards.
    ///
    /// ### Authority
    /// - `position_authority` - authority that owns the token corresponding to
    ///   this desired position.
    ///
    /// ### Parameters
    /// - `liquidity_amount` - The total amount of Liquidity the user is willing
    ///   to deposit.
    ...
    /// - `TokenMaxExceeded` - The required token to perform this operation
    ...
    /// Withdraw liquidity from a position in the Whirlpool. This call also
    /// updates the position's accrued fees and rewards.
    ///
    /// ### Authority
    /// - `position_authority` - authority that owns the token corresponding to
    ///   this desired position.
    ///
    /// ### Parameters
    /// - `liquidity_amount` - The total amount of Liquidity the user desires to
    ///   withdraw.
    /// - `token_min_a` - The minimum amount of tokenA the user is willing to
    ///   withdraw.
    /// - `token_min_b` - The minimum amount of tokenB the user is willing to
    ///   withdraw.
    ///
    /// #### Special Errors
    /// - `LiquidityZero` - Provided liquidity amount is zero.
    ...
    ///
    /// #### Special Errors
    /// - `TickNotFound` - Provided tick array account does not contain the tick
    ///   for this position.
    /// - `LiquidityZero` - Position has zero liquidity and therefore already
    ///   has the most updated fees and reward values.
    ...
    ///
    /// ### Authority
    /// - "token_authority" - The authority to withdraw tokens from the input
    ///   token account.
    ///
    /// ### Parameters
    /// - `amount` - The amount of input or output token to swap from (depending
    ///   on amount_specified_is_input).
    /// - `other_amount_threshold` - The maximum/minimum of input/output token
    ///   to swap into (depending on amount_specified_is_input).
    ...
    ///   `amount`represents. If true, the amount represents the input token of
    ///   the swap.
    /// - `a_to_b` - The direction of the swap. True if swapping from A to B.
    ...
    /// - `TickArrayIndexOutofBounds` - The swap loop attempted to access an
    ///   invalid array index during tick crossing.
    /// - `LiquidityOverflow` - Liquidity value overflowed 128bits during tick
    ///   crossing.
    /// - `InvalidTickSpacing` - The swap pool was initialized with tick-spacing
    ///   of 0.
    pub fn swap(
        ctx: Context<Swap>,
        amount: u64,
    ...
    /// Only the current fee authority has permission to invoke this
    /// instruction.
    ///
    /// ### Authority
    /// - "fee_authority" - Set authority in the WhirlpoolConfig
    ...
    /// - `FeeRateMaxExceeded` - If the provided default_fee_rate exceeds
    ///   MAX_FEE_RATE.
    pub fn set_default_fee_rate(ctx: Context<SetDefaultFeeRate>, default_fee_rate: u16) -> Result<()> {
    ...
    ///
    /// #### Special Errors
    /// - `ProtocolFeeRateMaxExceeded` - If the provided
    ///   default_protocol_fee_rate exceeds MAX_PROTOCOL_FEE_RATE.
    ...
    /// - `ProtocolFeeRateMaxExceeded` - If the provided
    ///   default_protocol_fee_rate exceeds MAX_PROTOCOL_FEE_RATE.
    pub fn set_protocol_fee_rate(ctx: Context<SetProtocolFeeRate>, protocol_fee_rate: u16) -> Result<()> {
        instructions::set_protocol_fee_rate::handler(ctx, protocol_fee_rate)
    }

    /// Sets the fee authority for a WhirlpoolConfig.
    /// The fee authority can set the fee & protocol fee rate for individual
    /// pools or set the default fee rate for newly minted pools.
    /// Only the current fee authority has permission to invoke this
    /// instruction.
    ///
    /// ### Authority
    ...
    ///   WhirlpoolConfig
    pub fn set_collect_protocol_fees_authority(ctx: Context<SetCollectProtocolFeesAuthority>) -> Result<()> {
    ...
    /// Only the current reward authority for this reward index has permission
    /// to invoke this instruction.
    ///
    /// ### Authority
    /// - "reward_authority" - Set authority that can control reward emission
    ...
    /// `WhirlpoolRewardInfo` whirlpool rewards.
    ///
    /// ### Authority
    /// - "reward_emissions_super_authority" - Set authority that can control
    ///   reward authorities for all pools in this config space.
    pub fn set_reward_emissions_super_authority(ctx: Context<SetRewardEmissionsSuperAuthority>) -> Result<()> {
    ...
    ///   `amount`represents. If true, the amount represents the input token of
    ///   the swap.
    /// - `a_to_b_one` - The direction of the swap of hop one. True if swapping
    ///   from A to B. False if swapping from B to A.
    /// - `a_to_b_two` - The direction of the swap of hop two. True if swapping
    ///   from A to B. False if swapping from B to A.
    /// - `sqrt_price_limit_one` - The maximum/minimum price the swap will swap
    ///   to in the first hop.
    /// - `sqrt_price_limit_two` - The maximum/minimum price the swap will swap
    ///   to in the second hop.
    ///
    /// #### Special Errors
    /// - `ZeroTradableAmount` - User provided parameter `amount` is 0.
    /// - `InvalidSqrtPriceLimitDirection` - User provided parameter
    ...
    /// - `InvalidTickSpacing` - The swap pool was initialized with tick-spacing
    ///   of 0.
    /// - `InvalidIntermediaryMint` - Error if the intermediary mint between hop
    ///   one and two do not equal.
    /// - `DuplicateTwoHopPool` - Error if whirlpool one & two are the same
    ///   pool.
    ...
    /// A unique token will be minted to represent the position bundle in the
    /// users wallet.
    ...
    /// Initializes a PositionBundle account that bundles several positions.
    /// A unique token will be minted to represent the position bundle in the
    /// users wallet. Additional Metaplex metadata is appended to identify
    /// the token.
    pub fn initialize_position_bundle_with_metadata(ctx: Context<InitializePositionBundleWithMetadata>) -> Result<()> {
    ...
    ///
    /// ### Authority
    /// - `position_bundle_owner` - The owner that owns the position bundle
    ///   token.
    ///
    /// ### Special Errors
    ...
    /// because the owner of the position bundle becomes the owner of the
    /// position. The position will start off with 0 liquidity.
    ///
    /// ### Authority
    /// - `position_bundle_authority` - authority that owns the token
    ///   corresponding to this desired position bundle.
    ///
    /// ### Parameters
    ...
    pub fn open_bundled_position(
        ...
        instructions::open_bundled_position::handler(ctx, bundle_index, tick_lower_index, tick_upper_index)
    }
    ...
    /// Open a position in a Whirlpool. A unique token will be minted to
    /// represent the position in the users wallet. Additional TokenMetadata
    /// extension is initialized to identify the token.
    ...
    /// - `tick_upper_index` - The tick specifying the upper end of the position
    ///   range.
    /// - `with_token_metadata_extension` - If true, the token metadata
    ///   extension will be initialized.
    ///
    /// #### Special Errors
    /// - `InvalidTickIndex` - If a provided tick is out of bounds, out of order
    ///   or not a multiple of the tick-spacing in this pool.
    ...
    /// Close a position in a Whirlpool. Burns the position token in the owner's
    /// wallet. Mint and TokenAccount are based on Token-2022. And Mint
    /// accout will be also closed.
    ///
    /// ### Authority
    /// - "position_authority" - The authority that owns the position token.
    ///
    ...
    /// WhirlpoolConfig space.
    ///
    /// ### Authority
    /// - "fee_authority" - Set authority in the WhirlpoolConfig
    ///
    /// ### Parameters
    /// - `fee_tier_index` - The index of the fee-tier that this adaptive fee
    ///   tier will be initialized.
    /// - `tick_spacing` - The tick-spacing that this fee-tier suggests the
    ///   default_fee_rate for.
    /// - `initialize_pool_authority` - The authority that can initialize pools
    ///   with this adaptive fee-tier.
    /// - `delegated_fee_authority` - The authority that can set the base fee
    ///   rate for pools using this adaptive fee-tier.
    /// - `default_fee_rate` - The default fee rate that a pool will use if the
    ///   pool uses this fee tier during initialization.
    /// - `filter_period` - Period determine high frequency trading time window.
    ///   (seconds)
    /// - `decay_period` - Period determine when the adaptive fee start
    ///   decrease. (seconds)
    ...
    /// - `major_swap_threshold_ticks` - Major swap threshold ticks to define
    ///   major swap.
    ///
    /// #### Special Errors
    /// - `InvalidTickSpacing` - If the provided tick_spacing is 0.
    ...
    /// - `FeeRateMaxExceeded` - If the provided default_fee_rate exceeds
    ///   MAX_FEE_RATE.
    /// - `InvalidAdaptiveFeeConstants` - If the provided adaptive fee constants
    ///   are invalid.
    #[allow(clippy::too_many_arguments)]
    ...
    /// Only the current fee authority in WhirlpoolsConfig has permission to
    ...
    ///
    /// ### Parameters
    /// - `default_base_fee_rate` - The default base fee rate that a pool will
    ///   use if the pool uses this adaptive fee-tier during initialization.
    ///
    /// #### Special Errors
    /// - `FeeRateMaxExceeded` - If the provided default_fee_rate exceeds
    ///   MAX_FEE_RATE.
    pub fn set_default_base_fee_rate(ctx: Context<SetDefaultBaseFeeRate>, default_base_fee_rate: u16) -> Result<()> {
    ...
    /// initialized with the adaptive fee-tier. Only the current fee
    /// authority in WhirlpoolsConfig has permission to invoke this instruction.
    ///
    /// ### Authority
    /// - "fee_authority" - Set authority in the WhirlpoolConfig
    ...
    /// Only the initialize pool authority can initialize pools with the
    /// adaptive fee-tier. Only the current fee authority in
    /// WhirlpoolsConfig has permission to invoke this instruction.
    ///
    /// ### Authority
    /// - "fee_authority" - Set authority in the WhirlpoolConfig
    ...
    /// ### Parameters
    /// - `initial_sqrt_price` - The desired initial sqrt-price for this pool
    /// - `trade_enable_timestamp` - The timestamp when trading is enabled for
    ///   this pool (within 72 hours)
    ///
    /// #### Special Errors
    /// `InvalidTokenMintOrder` - The order of mints have to be ordered by
    /// `SqrtPriceOutOfBounds` - provided initial_sqrt_price is not between
    /// 2^-64 to 2^64 `InvalidTradeEnableTimestamp` - provided
    /// trade_enable_timestamp is not within 72 hours or the adaptive fee-tier
    /// is permission-less `UnsupportedTokenMint` - The provided token mint
    /// is not supported by the program (e.g. it has risky token extensions)
    ...
    /// Sets the fee rate for a Whirlpool by the delegated fee authority in
    /// AdaptiveFeeTier. Fee rate is represented as hundredths of a basis
    /// point.
    ///
    /// ### Authority
    /// - "delegated_fee_authority" - Set authority that can modify pool fees in
    ///   the AdaptiveFeeTier
    ///
    /// ### Parameters
    /// - `fee_rate` - The rate that the pool will use to calculate fees going
