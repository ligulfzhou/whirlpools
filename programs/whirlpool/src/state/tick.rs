use {
    super::Whirlpool,
    crate::{errors::ErrorCode, state::NUM_REWARDS},
    anchor_lang::prelude::*,
};
...
impl Tick {
    ...
    ///
    /// # Returns
    /// - `true`: The tick index is not within the range supported by this
    ///   contract
    ...
    /// Check that the tick index is a valid start tick for a tick array in this
    /// whirlpool A valid start-tick-index is a multiple of tick_spacing &
    ...
    /// - `false`: The tick index is not a valid start-tick-index for this
    ///   whirlpool or the tick index not within the range supported by this
    ///   contract
    pub fn check_is_valid_start_tick(tick_index: i32, tick_spacing: u16) -> bool {
        let ticks_in_array = TICK_ARRAY_SIZE * tick_spacing as i32;

        if Tick::check_is_out_of_bounds(tick_index) {
            // Left-edge tick-array can have a start-tick-index smaller than the min tick
            // index
    ...
    /// Check that the tick index is within bounds and is a usable tick index
    /// for the given tick spacing.
    ///
    /// # Parameters
    /// - `tick_index` - A i32 integer representing the tick index
    /// - `tick_spacing` - A u8 integer of the tick spacing for this whirlpool
    ///
    /// # Returns
    /// - `true`: The tick index is within max/min index bounds for this
    ///   protocol and is a usable tick-index given the tick-spacing
    /// - `false`: The tick index is out of bounds or is not a usable tick for
    ///   this tick-spacing
    ...
    ///
    /// # Returns
    /// - `i32` The input tick index value but bounded by the max/min value of
    ///   this protocol.
...
pub trait TickArrayType {
    ...
    fn get_next_init_tick_index(&self, tick_index: i32, tick_spacing: u16, a_to_b: bool) -> Result<Option<i32>>;

    fn get_tick(&self, tick_index: i32, tick_spacing: u16) -> Result<&Tick>;

    fn update_tick(&mut self, tick_index: i32, tick_spacing: u16, update: &TickUpdate) -> Result<()>;

    /// Checks that this array holds the next tick index for the current tick
    /// index, given the pool's tick spacing & search direction.
    ///
    /// unshifted checks on [start, start + TICK_ARRAY_SIZE * tick_spacing)
    /// shifted checks on [start - tick_spacing, start + (TICK_ARRAY_SIZE - 1) *
    /// tick_spacing) (adjusting range by -tick_spacing)
    ///
    /// shifted == !a_to_b
    ///
    /// For a_to_b swaps, price moves left. All searchable ticks in this
    /// tick-array's range will end up in this tick's usable ticks.
    /// The search range is therefore the range of the tick-array.
    ///
    /// For b_to_a swaps, this tick-array's left-most ticks can be the 'next'
    /// usable tick-index of the previous tick-array. The right-most ticks
    /// also points towards the next tick-array. The search range is therefore
    /// shifted by 1 tick-spacing.
...
impl TickArray {
    ...
    /// - `start_tick_index` - A u8 integer of the tick spacing for this
    ///   whirlpool
    ///
    /// # Errors
    /// - `InvalidStartTick`: - The provided start-tick-index is not an
    ///   initializable tick index in this Whirlpool w/ this tick-spacing.
    pub fn initialize(&mut self, whirlpool: &Account<Whirlpool>, start_tick_index: i32) -> Result<()> {
...
impl TickArrayType for TickArray {
    ...
    ///
    /// # Parameters
    /// - `tick_index` - A i32 integer representing the tick index to start
    ///   searching for
    /// - `tick_spacing` - A u8 integer of the tick spacing for this whirlpool
    ...
    /// - `InvalidTickArraySequence` - error if `tick_index` is not a valid
    ///   search tick for the array
    ...
    fn get_next_init_tick_index(&self, tick_index: i32, tick_spacing: u16, a_to_b: bool) -> Result<Option<i32>> {
        ...
        // For a_to_b searches, the search moves to the left. The next possible
        // init-tick can be the 1st tick in the current offset For b_to_a
        // searches, the search moves to the right. The next possible init-tick cannot
        // be within the current offset
        if !a_to_b {
            curr_offset += 1;
        }
...
impl TickArrayType for ZeroedTickArray {
    ...
    fn update_tick(&mut self, _tick_index: i32, _tick_spacing: u16, _update: &TickUpdate) -> Result<()> {
...
pub mod tick_builder {
    use {super::Tick, crate::state::NUM_REWARDS};
    ...
    impl TickBuilder {
        ...
        pub fn reward_growths_outside(mut self, reward_growths_outside: [u128; NUM_REWARDS]) -> Self {
...
mod check_is_valid_start_tick_tests {
    ...
    fn test_min_tick_array_start_tick_is_valid_ts8() {
        ...
        let expected_start_index_for_last_array: i32 = expected_array_index * TICK_ARRAY_SIZE * TS_8 as i32;
    ...
    fn test_min_tick_array_start_tick_is_valid_ts128() {
        ...
        let expected_start_index_for_last_array: i32 = expected_array_index * TICK_ARRAY_SIZE * TS_128 as i32;
        assert!(Tick::check_is_valid_start_tick(
            expected_start_index_for_last_array,
            TS_128
        ))
...
mod full_range_indexes_tests {
    use {super::*, crate::math::FULL_RANGE_ONLY_TICK_SPACING_THRESHOLD};

    #[test]
    fn test_min_tick_spacing() {
        assert_eq!(Tick::full_range_indexes(1), (MIN_TICK_INDEX, MAX_TICK_INDEX));
    }
...
mod array_update_tests {
    ...
    fn update_applies_successfully() {
        ...
        array.update_tick(tick_index, tick_spacing, &update).unwrap();
