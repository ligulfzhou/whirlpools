use {
    crate::{errors::ErrorCode, state::*, util::ProxiedTickArray},
    anchor_lang::prelude::*,
    std::cell::RefMut,
};
...
impl<'info> SwapTickSequence<'info> {
    ...
    ///
    /// # Parameters
    /// - `array_index` - the array index that the tick of this given tick-index
    ///   would be stored in
    ...
    /// - `TickArrayIndexOutofBounds` - The provided array-index is out of
    ///   bounds
    /// - `TickNotFound`: - The provided tick-index is not an initializable tick
    ///   index in this Whirlpool w/ this tick-spacing.
    ...
    /// - `tick_index` - the tick index the desired Tick object is stored in
    /// - `tick_spacing` - A u8 integer of the tick spacing for this whirlpool
    /// - `update` - A reference to a TickUpdate object to update the Tick
    ...
    pub fn get_tick_offset(&self, array_index: usize, tick_index: i32, tick_spacing: u16) -> Result<isize> {
        let array = self.arrays.get(array_index);
        match array {
    ...
    /// - `tick_spacing` - A u8 integer of the tick spacing for this whirlpool
    /// - `a_to_b` - If the trade is from a_to_b, the search will move to the
    ///   left and the starting search tick is inclusive. If the trade is from
    ///   b_to_a, the search will move to the right and the starting search tick
    ///   is not inclusive.
    /// - `start_array_index` -
    ///
    /// # Returns
    /// - `(usize, i32, &mut Tick)`: The array_index which the next initialized
    ///   index was found, the next initialized tick-index & a mutable reference
    ///   to that tick
    /// - `TickArraySequenceInvalidIndex` - The swap loop provided an invalid
    ///   array index to query the next tick in.
    ...
    pub fn get_next_initialized_tick_index(
        ...
        // Keep looping the arrays until an initialized tick index in the subsequent
        // tick-arrays found.
        loop {
            // If we get to the end of the array sequence and next_index is still not found,
            // throw error
...
mod swap_tick_sequence_tests {
    ...
    fn build_tick_array(start_tick_index: i32, initialized_offsets: Vec<usize>) -> RefCell<TickArray> {
    ...
    mod modify_ticks {
        ...
        fn modify_tick_init_tick() {
            ...
            let mut swap_tick_sequence =
                SwapTickSequence::new(ta0.borrow_mut(), Some(ta1.borrow_mut()), Some(ta2.borrow_mut()));
            ...
            for init_tick_offset in initialized_ticks_offsets {
                ...
                let tick_index =
                    11264 - array_index as i32 * TS_128 as i32 * TICK_ARRAY_SIZE + init_tick_offset.1 * TS_128 as i32;
                ...
                let get_updated_result = swap_tick_sequence.get_tick(array_index, tick_index, TS_128).unwrap();
        ...
        fn modify_tick_uninitializable_tick() {
            ...
            for uninitializable_search_tick in uninitializable_tick_indices {
                let result =
                    swap_tick_sequence.get_tick(uninitializable_search_tick.0, uninitializable_search_tick.1, TS_128);
        ...
        fn modify_tick_uninitialized_tick() {
            ...
            for uninitializable_search_tick in uninitialized_tick_indices {
                ...
                let get_updated_result = swap_tick_sequence
                    .get_tick(uninitializable_search_tick.0, uninitializable_search_tick.1, TS_128)
                    .unwrap();
        ...
        fn cannot_modify_invalid_array_index() {
            ...
            assert_eq!(get_result.unwrap_err(), ErrorCode::TickArrayIndexOutofBounds.into());

            let update_result = swap_tick_sequence.update_tick(3, 5000, TS_128, &TickUpdate { ..Default::default() });
            assert_eq!(update_result.unwrap_err(), ErrorCode::TickArrayIndexOutofBounds.into());
        }
    }
    ...
    mod a_to_b {
        ...
        #[test]
        /// In an a_to_b search, the search-range of a tick-array is between 0 &
        /// last-tick - 1
        fn search_range() {
        ...
        #[test]
        /// On a b_to_a search where the search_index is within [-tickSpacing,
        /// 0) and search array begins at 0, correctly uses 0 as next
        /// initialized. This test is shifted by TICK_ARRAY_SIZE * TS_128.
        fn search_range_on_left() {
            let ta0 = build_tick_array(TICK_ARRAY_SIZE * TS_128 as i32, vec![0, 1, LAST_TICK_OFFSET]);
            let swap_tick_sequence = SwapTickSequence::new(ta0.borrow_mut(), None, None);

            // Verify start range is ok at start-tick-index
            let (start_range_array_index, start_range_result_index) = swap_tick_sequence
                .get_next_initialized_tick_index(TICK_ARRAY_SIZE * (TS_128 as i32) - 40, TS_128, false, 0)
                .unwrap();
        ...
        /// In an a_to_b search, search will panic if search index is on the
        /// last tick in array + 1
    ...
    mod b_to_a {
        ...
        #[test]
        /// In an b_to_a search, the search-range of a tick-array is between the
        /// last usable tick in the last array & the last usable tick in
        /// this array minus one.
        fn search_range() {
            let ta0 = build_tick_array(0, vec![0, LAST_TICK_OFFSET]);
            let swap_tick_sequence = SwapTickSequence::new(ta0.borrow_mut(), None, None);
        ...
        #[test]
        #[should_panic(expected = "InvalidTickArraySequence")]
        /// In an b_to_a search, search will panic if search index is on the
        /// last usable tick
        fn range_panic_on_end_range_plus_one() {
            let ta0 = build_tick_array(0, vec![0, LAST_TICK_OFFSET]);
            let swap_tick_sequence = SwapTickSequence::new(ta0.borrow_mut(), None, None);
            ...
            let (_, _) = swap_tick_sequence
                .get_next_initialized_tick_index(last_searchable_tick_in_array_plus_one, TS_8, false, 0)
                .unwrap();
        }

        #[test]
        #[should_panic(expected = "InvalidTickArraySequence")]
        /// In an b_to_a search, search will panic if search index is less than
        /// the last usable tick in the previous tick-array
        fn range_panic_on_start_range_sub_one() {
            let ta0 = build_tick_array(0, vec![0, LAST_TICK_OFFSET]);
            let swap_tick_sequence = SwapTickSequence::new(ta0.borrow_mut(), None, None);
    ...
    mod tick_bound {
        ...
        /// SwapTickSequence will bound the ticks by tick-array, not max/min
        /// tick. This is to reduce duplicated responsibility
        ...
        fn b_to_a_search_reaching_max_tick() {
            ...
            let ta2 = build_tick_array(443520, vec![]); // Max(443636).div_floor(tick-spacing (8) * TA Size (72))* tick-spacing (8) *
                                                        // TA Size (72)
            let swap_tick_sequence =
                SwapTickSequence::new(ta0.borrow_mut(), Some(ta1.borrow_mut()), Some(ta2.borrow_mut()));
        ...
        fn a_to_b_search_reaching_min_tick() {
            let ta0 = build_tick_array(0, vec![]);
            let ta1 = build_tick_array(0, vec![]);
            let ta2 = build_tick_array(-444096, vec![]); // Min(-443636).div_ceil(tick-spacing (8) * TA Size (72)) * tick-spacing (8) *
                                                         // TA Size (72)
            let swap_tick_sequence =
                SwapTickSequence::new(ta2.borrow_mut(), Some(ta1.borrow_mut()), Some(ta0.borrow_mut()));
    ...
    #[test]
    /// Search index on an initialized tick index will return that tick in a
    /// a_to_b search Expect:
    ///     - The same tick will be returned if search index is an initialized
    ///       tick
    fn a_to_b_search_on_initialized_index() {
        ...
        let tick = swap_tick_sequence.get_tick(array_index, index, TS_128).unwrap();
    ...
    ///
    /// Verifies:
    ///     - Search index will not return previous initialized indicies in a
    ///       b_to_a search
    ///     - If the search reaches the end of the last tick array, return the
    ///       first tick index of the last tick array
    fn a_to_b_search_entire_range() {
        ...
        for expected in expectation.iter() {
            ...
            // users on a_to_b search must manually decrement since a_to_b is inclusive of
            // current-tick
    ...
    ///
    /// Verifies:
    ///     - Search index on an initialized tick index will not return that
    ///       tick in a b_to_a search
    ///     - Search index will not return previous initialized indicies in a
    ///       b_to_a search
    ///     - Search indicies within the shifted range (-tick-spacing prior to
    ///       the start-tick) will return a valid initialized tick
    ///     - If the search reaches the last tick array, return the last tick in
    ///       the last tick array
    ...
    #[test]
    #[should_panic(expected = "InvalidTickArraySequence")]
    /// The starting point of a swap should always be contained within the first
    /// array Expected:
    ...
    fn array_0_out_of_sequence() {
        ...
        let swap_tick_sequence =
            SwapTickSequence::new(ta1.borrow_mut(), Some(ta0.borrow_mut()), Some(ta2.borrow_mut()));
    ...
    ///     - Panic on InvalidTickArraySequence when search-sequence is not in
    ///       array 1's range
    fn array_1_out_of_sequence() {
        ...
        let swap_tick_sequence =
            SwapTickSequence::new(ta2.borrow_mut(), Some(ta0.borrow_mut()), Some(ta1.borrow_mut()));
        ...
        for expected in expectation.iter() {
            ...
            let tick = swap_tick_sequence.get_tick(array_index, index, TS_8).unwrap();
            assert_eq!(tick.initialized, expected.2);
