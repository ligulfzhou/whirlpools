use {
    crate::{
        ...
        state::{
            AdaptiveFeeConstants, AdaptiveFeeInfo, AdaptiveFeeVariables, ADAPTIVE_FEE_CONTROL_FACTOR_DENOMINATOR,
            MAX_TICK_INDEX, MIN_TICK_INDEX, VOLATILITY_ACCUMULATOR_SCALE_FACTOR,
        },
    },
    anchor_lang::prelude::*,
};

// max fee rate should be controlled by max_volatility_accumulator, so this is a
// hard limit for safety. Fee rate is represented as hundredths of a basis
// point.
...
impl FeeRateManager {
    pub fn new(
        ...
        match adaptive_fee_info {
            ...
            Some(adaptive_fee_info) => {
                let tick_group_index =
                    floor_division(current_tick_index, adaptive_fee_info.constants.tick_group_size as i32);
                ...
                // max_volatility_accumulator < volatility_reference + tick_group_index_delta *
                // VOLATILITY_ACCUMULATOR_SCALE_FACTOR
                // -> ceil((max_volatility_accumulator - volatility_reference) /
                // VOLATILITY_ACCUMULATOR_SCALE_FACTOR) < tick_group_index_delta
                // From the above, if tick_group_index_delta is sufficiently large,
                // volatility_accumulator always sticks to max_volatility_accumulator
                let max_volatility_accumulator_tick_group_index_delta = ceil_division_u32(
                    adaptive_fee_constants.max_volatility_accumulator - adaptive_fee_variables.volatility_reference,
                    VOLATILITY_ACCUMULATOR_SCALE_FACTOR as u32,
                );

                // we need to calculate the adaptive fee rate for each tick_group_index in the
                // range of core tick group
                let core_tick_group_range_lower_index = adaptive_fee_variables.tick_group_index_reference -
                    max_volatility_accumulator_tick_group_index_delta as i32;
                let core_tick_group_range_upper_index = adaptive_fee_variables.tick_group_index_reference +
                    max_volatility_accumulator_tick_group_index_delta as i32;
                let core_tick_group_range_lower_bound_tick_index =
                    core_tick_group_range_lower_index * adaptive_fee_constants.tick_group_size as i32;
                ...
                let core_tick_group_range_lower_bound = if core_tick_group_range_lower_bound_tick_index > MIN_TICK_INDEX
                {
                    Some((
                ...
                let core_tick_group_range_upper_bound = if core_tick_group_range_upper_bound_tick_index < MAX_TICK_INDEX
                {
                    Some((
                ...
                // Note: reduction uses the value of volatility_accumulator, but
                // update_reference does not update it.
                //       update_volatility_accumulator is always called if the swap loop is
                // executed at least once,       amount == 0 and
                // sqrt_price_limit == whirlpool.sqrt_price are rejected, so the loop is
    ...
    pub fn update_volatility_accumulator(&mut self) -> Result<()> {
        match self {
            ...
            Self::Adaptive {
                ...
                adaptive_fee_variables,
                ..
            } => adaptive_fee_variables.update_volatility_accumulator(*tick_group_index, adaptive_fee_constants),
        }
    }
        ...
        match self {
            ...
            Self::Adaptive {
                ...
                let (tick_index, is_on_tick_group_boundary) = if sqrt_price == next_tick_sqrt_price {
                    ...
                    // Note: It was pointed out during the review that using curr_tick_index may
                    // suppress tick_index_from_sqrt_price.       However, since
                    // curr_tick_index may also be shifted by -1, we decided to prioritize safety by
                    // recalculating it here.
                    let tick_index = tick_index_from_sqrt_price(&sqrt_price);
                    let is_on_tick_group_boundary = tick_index % adaptive_fee_constants.tick_group_size as i32 == 0 &&
                ...
                // In most cases, last_traversed_tick_group_index and tick_group_index are
                // expected to be different because of the skip. However, if the
                // skip only advances by 1 tick_spacing, they will be the same
                // (update_volatility_accumulator is updated at the beginning of the loop, so no
                // update is needed). If sqrt_price is on the tick group
                // boundary and has not advanced at all (all amount is collected as fees), we
                // need to prevent backward movement in the b to a direction. This is why we
                    ...
                    (!*a_to_b && last_traversed_tick_group_index > *tick_group_index)
                {
                    *tick_group_index = last_traversed_tick_group_index;
                    // volatility_accumulator is updated with the new tick_group_index based on new
                    // sqrt_price
                    adaptive_fee_variables.update_volatility_accumulator(*tick_group_index, adaptive_fee_constants)?;
                }

                // tick_group_index will be shifted to left(-1) or right(+1) for the next loop.
                // If sqrt_price is not on a tick_group_size boundary, shifting will advance too
                // much, but tick_group_index is not recorded in the chain and
                // the loop ends, so there is no adverse effect on subsequent processing.
                *tick_group_index += if *a_to_b { -1 } else { 1 };
    ...
    //
    // When skip is true, we need to call advance_tick_group_after_skip() instead of
    // advance_tick_group().
    pub fn get_bounded_sqrt_price_target(&self, sqrt_price: u128, curr_liquidity: u128) -> (u128, bool) {
        match self {
            ...
            Self::Adaptive {
                ...
                // the range where volatility_accumulator is always max_volatility_accumulator
                // can be skipped.
                ...
                let boundary_sqrt_price =
                    sqrt_price_from_tick_index(boundary_tick_index.clamp(MIN_TICK_INDEX, MAX_TICK_INDEX));
    ...
    fn compute_adaptive_fee_rate(
        ...
        let crossed = adaptive_fee_variables.volatility_accumulator * adaptive_fee_constants.tick_group_size as u32;
...
mod static_fee_rate_manager_tests {
    ...
    fn test_new() {
        ...
        match fee_rate_manager {
            FeeRateManager::Static { static_fee_rate: rate } => {
    ...
    fn test_get_bounded_sqrt_price_target() {
        ...
        fn check_not_bounded(fee_rate_manager: &FeeRateManager, sqrt_price: u128) {
            ...
            assert!(!skip); // skip should be always false for
                            // StaticFeeRateManager
        }

        check_not_bounded(&fee_rate_manager, sqrt_price_from_tick_index(MIN_TICK_INDEX));
        check_not_bounded(&fee_rate_manager, sqrt_price_from_tick_index(MIN_TICK_INDEX / 2));
        check_not_bounded(&fee_rate_manager, sqrt_price_from_tick_index(0));
        check_not_bounded(&fee_rate_manager, sqrt_price_from_tick_index(MAX_TICK_INDEX / 2));
        check_not_bounded(&fee_rate_manager, sqrt_price_from_tick_index(MAX_TICK_INDEX));
    }
...
mod adaptive_fee_rate_manager_tests {
    use {
        ...
        crate::{
            ...
            state::{AdaptiveFeeConstants, AdaptiveFeeInfo, AdaptiveFeeVariables, REDUCTION_FACTOR_DENOMINATOR},
        },
    };
    ...
    fn check_variables(
        ...
        assert!(adaptive_fee_variables.last_reference_update_timestamp == last_reference_update_timestamp);
    ...
    fn test_new() {
        ...
        match fee_rate_manager {
            FeeRateManager::Adaptive {
                ...
                // max_volatility_delta = ceil(  (max_volatility_accumulator -
                // volatility_reference) / VOLATILITY_ACCUMULATOR_SCALE_FACTOR )
                // = ceil( (350_000 - 500) / 10_000 ) = 35
                // tick_group_index_reference +/- max_volatility_delta
                // = 1 +/- 35 = -34, 36
                assert_eq!(
                    core_tick_group_range_lower_bound,
                    Some((-34, sqrt_price_from_tick_index(-34 * tick_group_size as i32)))
                );
                assert_eq!(
                    core_tick_group_range_upper_bound,
                    Some((
                        36,
                        sqrt_price_from_tick_index(36 * tick_group_size as i32 + tick_group_size as i32)
                    ))
                );
                ...
                check_variables(
                    ...
                    // volatility_accumulator should NOT be updated (it should be updated by
                    // update_volatility_accumulator function)
        ...
        match fee_rate_manager {
            FeeRateManager::Adaptive {
                ...
                assert_eq!(
                    core_tick_group_range_lower_bound,
                    Some((-19, sqrt_price_from_tick_index(-19 * tick_group_size as i32)))
                );
                assert_eq!(
                    core_tick_group_range_upper_bound,
                    Some((
                        51,
                        sqrt_price_from_tick_index(51 * tick_group_size as i32 + tick_group_size as i32)
                    ))
                );
                ...
                let expected_volatility_reference = adaptive_fee_info.variables.volatility_accumulator *
    ...
    mod test_new_core_tick_group_range {
        ...
        fn test(
            ...
            let expected_core_tick_group_range_lower_bound = expected_lower_tick_group_index
                .map(|index| (index, sqrt_price_from_tick_index(index * tick_group_size as i32)));
        ...
        fn test_ts_64() {
            ...
            // zero max volatility accumulator (edge case: should set adaptive fee factor to
            // 0 if adaptive fee is not used)
    ...
    mod test_compute_adaptive_fee_rate {
        ...
        fn test(constants: AdaptiveFeeConstants, pre_calculated_fee_rates: &[u32]) {
            ...
            for (delta, pre_calculated_fee_rate) in pre_calculated_fee_rates.iter().enumerate() {
                ...
                let volatility_accumulator = delta as u32 * VOLATILITY_ACCUMULATOR_SCALE_FACTOR as u32;
                let capped_volatility_accumulator = volatility_accumulator.min(constants.max_volatility_accumulator);

                let crossed_tick_indexes = capped_volatility_accumulator * constants.tick_group_size as u32;
                let squared_crossed_tick_indexes = u64::from(crossed_tick_indexes) * u64::from(crossed_tick_indexes);

                let expected_fee_rate = ceil_division_u128(
                    u128::from(constants.adaptive_fee_control_factor) * u128::from(squared_crossed_tick_indexes),
                    u128::from(ADAPTIVE_FEE_CONTROL_FACTOR_DENOMINATOR) *
                        u128::from(VOLATILITY_ACCUMULATOR_SCALE_FACTOR) *
                        u128::from(VOLATILITY_ACCUMULATOR_SCALE_FACTOR),
                ) as u32;
        ...
        fn test_max_volatility_accumulator_should_bound_fee_rate() {
            test(
                ...
                */
                &[
                    0, 62, 246, 553, 984, 1536, 2212, 3011, 3933, 4977, 6144, 7435, 8848, 10384, 12043, 13824, 15729,
                    17757, 19907, 22180, 24576, 27096, 29737, 32502, 35390, 38400, 41534, 44790, 48169, 51672, 55296,
                    59044, 62915, 66909, 71025, 75264, 75264, 75264, 75264, 75264, 75264, 75264, 75264, 75264, 75264,
                    75264, 75264, 75264, 75264, 75264,
                ],
            );
        }
            ...
            test(
                ...
                */
                &[
                    0, 62, 246, 553, 984, 1536, 2212, 3011, 3933, 4977, 6144, 7435, 8848, 10384, 12043, 13824, 15729,
                    17757, 19907, 22180, 24576, 27096, 29737, 32502, 35390, 38400, 41534, 44790, 48169, 51672, 55296,
                    59044, 62915, 66909, 71025, 75264, 79627, 84112, 88720, 93451, 98304, 100000, 100000, 100000,
        ...
        fn test_fee_rate_is_not_bounded_in_this_range() {
            test(
                ...
                */
                &[
                    0, 41, 164, 369, 656, 1024, 1475, 2008, 2622, 3318, 4096, 4957, 5899, 6923, 8029, 9216, 10486,
                    11838, 13272, 14787, 16384, 18064, 19825, 21668, 23593, 25600, 27689, 29860, 32113, 34448, 36864,
                    39363, 41944, 44606, 47350, 50176, 53085, 56075, 59147, 62301, 65536, 68854, 72254, 75736, 79299,
                    82944, 86672, 90481, 94372, 98345,
                ],
            );
        }
    }
    ...
    fn test_get_total_fee_rate() {
        ...
        */
        let pre_calculated_total_fee_rates = [
            10000, 10062, 10246, 10553, 10984, 11536, 12212, 13011, 13933, 14977, 16144, 17435, 18848, 20384, 22043,
            23824, 25729, 27757, 29907, 32180, 34576, 37096, 39737, 42502, 45390, 48400, 51534, 54790, 58169, 61672,
            65296, 69044, 72915, 76909, 81025, 85264, 89627, 94112, 98720, 100000, 100000, 100000, 100000, 100000,
            100000, 100000, 100000, 100000, 100000, 100000,
        ];
    ...
    fn test_get_bounded_sqrt_price_target_a_to_b_without_skip() {
        ...
        let (bounded_sqrt_price, skip) = fee_rate_manager.get_bounded_sqrt_price_target(sqrt_price, non_zero_liquidity);
        assert_eq!(bounded_sqrt_price, sqrt_price);
        assert!(!skip);
    ...
    fn test_get_bounded_sqrt_price_target_a_to_b_with_zero_liquidity_skip() {
        ...
        // sqrt_price is near than the boundary
        let sqrt_price = sqrt_price_from_tick_index(1024 + 16);
        let (bounded_sqrt_price, skip) = fee_rate_manager.get_bounded_sqrt_price_target(sqrt_price, zero_liquidity);
        assert_eq!(bounded_sqrt_price, sqrt_price);
    ...
    fn test_get_bounded_sqrt_price_target_b_to_a_without_skip() {
        ...
        let sqrt_price = MAX_SQRT_PRICE_X64;
        let (bounded_sqrt_price, skip) = fee_rate_manager.get_bounded_sqrt_price_target(sqrt_price, non_zero_liquidity);
        assert_eq!(bounded_sqrt_price, sqrt_price_from_tick_index(1024 + 64 + 64));
        assert!(!skip);
    }
    ...
    fn test_get_bounded_sqrt_price_target_b_to_a_with_max_volatility_skip() {
        ...
        let sqrt_price = MAX_SQRT_PRICE_X64;
        let (bounded_sqrt_price, skip) = fee_rate_manager.get_bounded_sqrt_price_target(sqrt_price, non_zero_liquidity);
        assert_eq!(bounded_sqrt_price, sqrt_price_from_tick_index(64 * -8 + 64 + 64)); // should be bounded
        assert!(!skip);
    ...
    mod advance_tick_group_after_skip {
        ...
        fn test(
            ...
            match fee_rate_manager {
                FeeRateManager::Adaptive {
                    ...
                    assert!(adaptive_fee_variables.volatility_accumulator == expected_volatility_accumulator);
                }
        ...
        fn test_b_to_a() {
            ...
            // In advance_tick_group_after_skip, tick_group_index will be shifted to right
            // by 1 for the next loop. If it is not a tick_group_size boundary,
            // shifting will advance too much, but tick_group_index is not
            // recorded in the chain and the loop ends, so there is no adverse effect on
            ...
            // -11264 --> tick(-11264) (out of core range, amount is collected as fee, no
            // price change)
            test(
                a_to_b,
                -11264,
        ...
        fn test_a_to_b() {
            ...
            // In advance_tick_group_after_skip, tick_group_index will be shifted to left by
            // 1 for the next loop. If it is not a tick_group_size boundary,
            // shifting will advance too much, but tick_group_index is not
            // recorded in the chain and the loop ends, so there is no adverse effect on
            // subsequent processing.
            ...
            // tick(11264) <-- 11264 (out of core range, amount is collected as fee, no
            // price change)
            test(
                a_to_b,
                11264,
    ...
    mod test_update_major_swap_timestamp {
        ...
        fn test_a_to_b_minor() {
            ...
            let post_sqrt_price =
                sqrt_price_from_tick_index(current_tick_index - major_swap_threshold_ticks as i32 + 1);

            fee_rate_manager
                .update_major_swap_timestamp(timestamp, pre_sqrt_price, post_sqrt_price)
                .unwrap();
            let next_variables = fee_rate_manager.get_next_adaptive_fee_info().unwrap().variables;

            // should not be updated
            assert!(next_variables.last_major_swap_timestamp == adaptive_fee_info.variables.last_major_swap_timestamp);
